...
- GitHub Actions on `main` generates Templ, builds Tailwind, runs `make docker-buildx`, then force-pushes to the configured local bare repo (default `/srv/git/learnd-ci.git`) to trigger deployment.
